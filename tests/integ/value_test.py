...
class ValueTest(helper.TestCase):
    ...
    def testRank1DatasetBinary(self):
        ...
        headers_bin_req = helper.getRequestHeaders(domain=self.domain)
        headers_bin_req["Content-Type"] = aos
        headers_bin_rsp = helper.getRequestHeaders(domain=self.domain)
        ...
        # read values from dset (should be zeros)
        req = f"{self.endpoint}/datasets/{dset_id}/value" 
        rsp = GET(req, headers=headers_bin_rsp)
        ...
        for i in range(array_len):
            data[i*4] = i
        rsp = PUT(req, data=data, headers=headers_bin_req)
        self.assertEqual(rsp.status_code, 200)
        ...
        rsp = GET(req, params=params, headers=headers_bin_rsp)
        self.assertEqual(rsp.status_code, 200)
        data = rsp.content
        self.assertEqual(len(data), 4*6)
        for i in range(6):
            offset = i*4
            val = 255 if i >= 4 else i # if updated by previous selection
            self.assertEqual(data[offset+0], val)
            self.assertEqual(data[offset+1], 0)
            self.assertEqual(data[offset+2], 0)
            self.assertEqual(data[offset+3], 0)
    ...
    def test2DDatasetBinary(self):
        ...
        for i in range(num_row):
            ...
            json_data.append(row)
        rsp = PUT(req, data=bin_data, headers=headers_bin_req)
        self.assertEqual(rsp.status_code, 200)
    ...
    def testSelection1DDataset(self):
        # create dataset
        dset_body = { "type": "H5T_STD_I32LE", "shape": 10 }
        dset_id = helper.postDataset(self.domain, dset_body)
    ...
    def testSelection2DDataset(self):
        # create dataset
        ...
        dset_id = helper.postDataset(self.domain, payload)
    ...
    def testNullPadStringBinary(self):
    ...
    def testOverflowFixedString(self):
    ...
    def testCompound(self):
        ...
        dset0d_uuid = helper.postDataset(self.domain, payload)
        ...
        dset1d_uuid = helper.postDataset(self.domain, payload)
        ...
        dset2d_uuid = helper.postDataset(self.domain, payload)
    ...
    def testObjRefDataset(self):
        # Test PUT obj ref values for 1d dataset

        # create new group  
        g1_uuid = helper.postGroup(self.domain)
        ref_values = [
            f"groups/{self.root_uuid}",
            '',
            f"groups/{g1_uuid}",
        ]
    ...
    def testObjRefDatasetBinary(self):
        ...
        ref_values = ["groups/" + self.root_uuid, '', "groups/" + g1_uuid]
        for i in range(array_len):
